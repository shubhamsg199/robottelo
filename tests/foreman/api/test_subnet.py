...
def test_positive_add_parameter(name, target_sat):
    ...
    subnet_param = target_sat.api.Parameter(
        subnet=subnet.id, name=name, value=value, parameter_type='string'
    ).create()
...
def test_positive_add_parameter_with_values_and_separator(target_sat):
    ...
    subnet_param = target_sat.api.Parameter(
        name=name, subnet=subnet.id, value=values, parameter_type='string'
    ).create()
    assert subnet_param.name == name
...
def test_positive_create_with_parameter_and_valid_separator(separator, target_sat):
    ...
    value = gen_string('utf8')
    subnet_param = target_sat.api.Parameter(
        name=name, subnet=subnet.id, value=value, parameter_type='string'
    ).create()
    assert subnet_param.name == name
    assert subnet_param.value == value
...
def test_negative_create_with_duplicated_parameters(target_sat):
    ...
    target_sat.api.Parameter(
        name='duplicateParameter', subnet=subnet.id, parameter_type='string'
    ).create()
...
def test_negative_update_parameter(new_name, target_sat):
    ...
    sub_param = target_sat.api.Parameter(
        name=gen_string('utf8'), subnet=subnet.id, value=gen_string('utf8'), parameter_type='string'
    ).create()
    with pytest.raises(HTTPError):
...
def test_positive_delete_subnet_parameter(target_sat):
    ...
    """
    subnet = target_sat.api.Subnet().create()
    sub_param = target_sat.api.Parameter(subnet=subnet.id, parameter_type='string').create()
    sub_param.delete()
    with pytest.raises(HTTPError):
        sub_param.read()
